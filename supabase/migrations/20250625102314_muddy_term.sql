...
DROP FUNCTION IF EXISTS update_booking_on_payment() CASCADE;
DROP FUNCTION IF EXISTS fix_booking(text) CASCADE;
DROP FUNCTION IF EXISTS fix_booking_status(text) CASCADE;
DROP VIEW IF EXISTS booking_status_check CASCADE;
...
-- Create a new function to manually fix bookings (with unique name)
CREATE OR REPLACE FUNCTION repair_booking_by_reference(booking_ref text)
RETURNS json AS $$
DECLARE
  booking_record RECORD;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN json_build_object(
      'success', false,
      'error', 'No payment found for booking: ' || booking_ref
    );
  END IF;
  ...
  SELECT json_build_object(
    ...
    'message', 'Booking repaired successfully'
  ) INTO result;
...
CREATE VIEW booking_status_check AS
SELECT 
  b.id as booking_id,
  b.booking_reference,
...
-- Fix all existing bookings with successful payments but inconsistent status
DO $$
DECLARE
  booking_record RECORD;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM bookings WHERE booking_reference = 'TRV-MCBS86EL-3EED0522') THEN
    SELECT repair_booking_by_reference('TRV-MCBS86EL-3EED0522') INTO fix_result;
...
COMMENT ON FUNCTION repair_booking_by_reference(text) IS 'Manually repair a specific booking by reference - returns JSON result';
