...
CREATE OR REPLACE FUNCTION fix_booking_status(booking_ref text)
RETURNS json AS $$
DECLARE
  booking_record RECORD;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM bookings WHERE booking_reference = 'TRV-MCBS86EL-3EED0522') THEN
    SELECT fix_booking_status('TRV-MCBS86EL-3EED0522') INTO fix_result;
...
COMMENT ON FUNCTION fix_booking_status(text) IS 'Manually fix a specific booking by reference - returns JSON result';
